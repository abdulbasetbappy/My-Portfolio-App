<script setup>
  ...
  useHead({
    titleTemplate: "Bappy · Design | %s (Design Problems Solution)"
  });

  const route = useRoute();

  const { data } = await useAsyncData("get-design-post", () =>
  ...
  const serverMeta = {
    ...
    ogLocale: "en_US",
    twitterCard: "summary",
    twitterCreator: "@abdulbasetbappy"
  };

  useSeoMeta({
    title: () => postTitle,
    description: () => postDescription,
    ogTitle: () => `${postTitle} By Skilled Frontend Developer Abdul Baset Bappy`,
    twitterTitle: () => `${postTitle} By Skilled Frontend Developer Abdul Baset Bappy`,
